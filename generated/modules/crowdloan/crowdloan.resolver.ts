...
export class CrowdloanResolver {
  ...
  @FieldResolver(() => Bid)
  async bidfund(@Root() r: Crowdloan, @Ctx() ctx: BaseContext): Promise<Bid[] | null> {
    return ctx.dataLoader.loaders.Crowdloan.bidfund.load(r);
  }

  @FieldResolver(() => Parachain)
  async parachainactiveFund(@Root() r: Crowdloan, @Ctx() ctx: BaseContext): Promise<Parachain[] | null> {
    return ctx.dataLoader.loaders.Crowdloan.parachainactiveFund.load(r);
  }
}
