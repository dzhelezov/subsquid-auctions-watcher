...
export class ParachainService extends HydraBaseService<Parachain> {
  ...
  @Inject('CrowdloanService')
  public readonly activeFundService!: CrowdloanService;
  @Inject('BidService')
  public readonly latestBidService!: BidService;
  ...
  buildFindWithRelationsQuery<W extends WhereInput>(
    ...
    // remove relation filters to enable warthog query builders
    const { activeFund } = where;
    delete where.activeFund;

    // remove relation filters to enable warthog query builders
    const { latestBid } = where;
    delete where.latestBid;
    ...
    if (activeFund) {
      // OTO or MTO
      const activeFundQuery = this.activeFundService
        .buildFindQueryWithParams(<any>activeFund, undefined, undefined, ['id'], 'activeFund')
        .take(undefined); // remove the default LIMIT

      mainQuery = mainQuery.andWhere(`"parachain"."active_fund_id" IN (${activeFundQuery.getQuery()})`);

      parameters = { ...parameters, ...activeFundQuery.getParameters() };
    }

    if (latestBid) {
      // OTO or MTO
      const latestBidQuery = this.latestBidService
        .buildFindQueryWithParams(<any>latestBid, undefined, undefined, ['id'], 'latestBid')
        .take(undefined); // remove the default LIMIT

      mainQuery = mainQuery.andWhere(`"parachain"."latest_bid_id" IN (${latestBidQuery.getQuery()})`);

      parameters = { ...parameters, ...latestBidQuery.getParameters() };
    }
