...
export class ChronicleResolver {
  ...
  async curAuction(@Root() r: Chronicle, @Ctx() ctx: BaseContext): Promise<Auction | null> {
    return ctx.dataLoader.loaders.Chronicle.curAuction.load(r);
  }

  @FieldResolver(() => Parachain)
  async parachains(@Root() r: Chronicle, @Ctx() ctx: BaseContext): Promise<Parachain[] | null> {
    return ctx.dataLoader.loaders.Chronicle.parachains.load(r);
  }
}
