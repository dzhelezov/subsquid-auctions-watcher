...
export class ContributionResolver {
  ...
  async parachain(@Root() r: Contribution, @Ctx() ctx: BaseContext): Promise<Parachain | null> {
    return ctx.dataLoader.loaders.Contribution.parachain.load(r);
  }

  @FieldResolver(() => Crowdloan)
  async fund(@Root() r: Contribution, @Ctx() ctx: BaseContext): Promise<Crowdloan | null> {
    return ctx.dataLoader.loaders.Contribution.fund.load(r);
  }
}
