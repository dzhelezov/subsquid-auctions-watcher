...
export class BidResolver {
  ...
  async auction(@Root() r: Bid, @Ctx() ctx: BaseContext): Promise<Auction | null> {
    return ctx.dataLoader.loaders.Bid.auction.load(r);
  }
  ...
  async fund(@Root() r: Bid, @Ctx() ctx: BaseContext): Promise<Crowdloan | null> {
    return ctx.dataLoader.loaders.Bid.fund.load(r);
  }

  @FieldResolver(() => Parachain)
  async parachainlatestBid(@Root() r: Bid, @Ctx() ctx: BaseContext): Promise<Parachain[] | null> {
    return ctx.dataLoader.loaders.Bid.parachainlatestBid.load(r);
  }
}
